...
{
    public partial class AddMyCarServicesComponent : ComponentBase
    {
        ...
        protected async System.Threading.Tasks.Task Load()
        {
            ...
            carservice = new SimpleCarManagment.Models.SimpleCarManagmentDb.CarService(){};

            carservice.CarId = carId;

            carservice.Date = DateTime.Now.Date;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(SimpleCarManagment.Models.SimpleCarManagmentDb.CarService args)
        {
            try
            {
                var simpleCarManagmentDbCreateCarServiceResult = await SimpleCarManagmentDb.CreateCarService(carservice);
                DialogService.Close(carservice);
            }
            catch (System.Exception simpleCarManagmentDbCreateCarServiceException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to create new CarService!" });
            }
        }
