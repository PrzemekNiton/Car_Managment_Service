...
{
    public partial class EditServicesComponent : ComponentBase
    {
        ...
        {
            var simpleCarManagmentDbGetServiceByIdResult = await SimpleCarManagmentDb.GetServiceById(Id);
            service = simpleCarManagmentDbGetServiceByIdResult;
        }
        ...
        protected async System.Threading.Tasks.Task Form0Submit(SimpleCarManagment.Models.SimpleCarManagmentDb.Service args)
        {
            try
            {
                var simpleCarManagmentDbUpdateServiceResult = await SimpleCarManagmentDb.UpdateService(Id, service);
                DialogService.Close(service);
            }
            catch (System.Exception simpleCarManagmentDbUpdateServiceException)
            {
                NotificationService.Notify(new NotificationMessage(){ Severity = NotificationSeverity.Error,Summary = $"Error",Detail = $"Unable to update Service" });
            }
        }
